...
Module['onRuntimeInitialized'] = function() {
    ...
    setInterval(function() {
        const ptr = Module['_nextLine']();
        if(ptr) {
            const data = Module['UTF8ToString'](ptr);
            Module['onStdout'](data);
        }
        Module['_free'](ptr);
    }, 20);
};
 ...
 */
Module['sendOsc'] =  (oscBytes) => {
        // copy bytes into the scsynth heap from a fresh view
        const ptr = Module['_malloc'](oscBytes.byteLength);
        new Uint8Array(Module.scHeap).set(oscBytes, ptr);
        Module['_scsynthSendOsc'](ptr, oscBytes.byteLength);
        Module['_free'](ptr);
};
